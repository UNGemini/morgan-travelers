...
function ensureRouteLayers() {
  if (!map.getSource("route-line")) {
    map.addSource("route-line", {
      type: "geojson",
      // Keep GTFS/OSRM vertices (default tolerance chords the line off-road).
      // Leave maxzoom at the MapLibre default (18) so long routes still tile.
  ...
  try {
    if (map.getLayer("route-line-main")) {
      ...
      map.setPaintProperty("route-line-main", "line-width", [
        "case",
        ["==", ["get", "walk_style"], "indoor"],
        3.5,
        ["==", ["get", "walk_style"], "free"],
    ...
    if (map.getLayer("route-line-casing")) {
      map.setPaintProperty("route-line-casing", "line-width", 8);
    }
    if (map.getLayer("route-stops-circle")) {
